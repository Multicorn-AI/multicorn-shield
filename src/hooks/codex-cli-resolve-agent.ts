 ...
 *
 * @module hooks/codex-cli-resolve-agent
 */
...
/**
 * Picks the agent for Codex CLI hooks: longest matching workspacePath under cwd, then defaultAgent, then first codex-cli agent.
 *
 * @param obj - Parsed ~/.multicorn/config.json
 * @param cwd - Prefer `process.env.PWD` when set (shell cwd); else caller passes `process.cwd()`.
 */
export function resolveCodexCliAgentNameFromConfig(
